...
<body>
	...
	// 2. bno null 이면 list.jsp 리다이렉트
	if (b == null) {
		response.sendRedirect("list.jsp");
		
	}else {
		
		// 3. bno null 이 아니면 숫자로 형변환
		int bno = Integer.parseInt(b);
		// 4. dao
		BoardDAO dao = new BoardDAO();
		// 5. dao를 통해서 지정한 게시물 가져오기
		BoardVO vo = dao.selectOne(bno);
		// 6. 화면에 표 형태로 출력 : 작성자 제목 내용
			...
			<form action="modifyOk.jsp">
				<table>
					<tr>
						<th>작성자</th>
						<td>
							<input type="text" name="writer" id="" value = ""/><%= vo.getWriter() %></td>
							<input type="hidden" name="bno" value = "<%= vo.getBno() %>" />
					</tr>
					
					<tr>
						<th>제목</th>
						<td><input type="text" name="title" id="" value = ""/><%= vo.getTitle() %></td>
					</tr>
